...
async function startGithubReauthentication(): Promise<void> {
  ...
  if (result.isErr()) {
    toast.failure(t('Failed to start GitHub reconnect flow'));
    return;
  }
...
export function GithubReauthenticationPrompt() {
  ...
  useKeyedPersistentToasts({
    ...
    key: (item) => item,
    toast: (_item, dismiss) => ({
      title: t('Reconnect GitHub'),
      content(): string {
        return t(
          'Your GitHub authorization has expired. Reconnect GitHub to restore pull request details.'
        );
      },
      actions: [
        {
