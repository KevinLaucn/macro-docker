...
async function run() {
      ...
      traverse(ast, {
        ...
        StringLiteral(p: any) {
          ...
          if (isUiFallbackStringLiteral(p) && shouldTranslateText(val)) {
            const dictKey = fileContext ? `${val}@@${fileContext}` : val;
            recordCall(dictKey, rel, fileContext, false, line, 'JSXFallback');
          }
  ...
  if (uiMissingCount > 0) {
    ...
    if (Object.keys(byFile).length > 10) {
      console.log(
        `  ... and ${Object.keys(byFile).length - 10} more files (see packages/fork/i18n/diff/missing-details.json)`
      );
    }
  }
  ...
  console.log(`\nDetailed reports saved in packages/fork/i18n/diff/`);
  console.log('==========================================');
}
